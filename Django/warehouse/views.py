...
from warehouse.models import Warehouse, Bigtable
...
def index(request):
    ...
    ################BIGTABLE################
    # with open("/Users/yitianliu/Downloads/bigtable.json", "r") as f:
...
# Experiment 3a: Single select by city
    ...
    #         res = list(Bigtable.objects.all().filter(gender=gender))
    #         count += len(res)

    # end = time.time()
    # print(f'Django Single SELECT by gender Rows/sec: {count/(end-start): .2f}')
...
# Experiment 3c: Single select by gender limit 20
    # limit = 20
    # count = 0
    # genders = [
    #     'Genderqueer',
    ...
    # return HttpResponse(f'Django bigtable Single SELECT by gender limit 20 Rows/sec: {count/(end-start): .2f}')
